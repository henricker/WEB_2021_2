...
export const TeacherUpdate: React.FC<TeacherCreateProps> = ({ teacherHookContext }: TeacherCreateProps) => {
  ...
  useEffect(() => {
    const teacherId = global.location.pathname.split('/').pop()
    const teacher = teacherHookContext.teachers.find(teacher => teacher._id === teacherId)!
    setData({ ...teacher })
  }, [teacherHookContext])
  ...
  function handleSubmit(event: React.FormEvent<any>) {
    ...
    teacherHookContext.updateTeacher(data._id!, data)
