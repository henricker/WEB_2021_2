          ...
          <tbody>
              {studentHookContext.students.map((student, index) => (
                  <tr key={index}>
                    ...
                    <Link to={`/students/update/${student._id}`} className='btn btn-primary'>Editar</Link>
                    </td>
                    <td style={{textAlign:'center'}}>
                      <button className='btn btn-danger' onClick={() => studentHookContext.deleteStudent(student._id!)}>Apagar</button>
                    </td>
                  </tr>
              ))}
          </tbody>
